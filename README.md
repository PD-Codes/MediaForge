...
- **Built-in eBook reader** — PDF, EPUB, MOBI, AZW3 and AZW open in a full-screen overlay instead of a browser tab (Mobipocket formats are converted to EPUB once, on the server, and cached), with light/sepia/dark reading palettes, adjustable text size, a table of contents and a choice between paged and scrolling reading. Where you stopped is remembered **per book rather than per file**, so starting a novel as EPUB and continuing in the PDF keeps your place
