...
- **Catalogue** — the complete A–Z list AniWorld and SerienStream publish (2.4k and 10.8k titles), with instant filtering, an A–Z jump rail, filters for what you already have, queued or auto-sync, A–Z/Z–A sorting, multi-select and a details view per title that marks which episodes are already on disk; the lists are kept in the database and refreshed in the background, so the page opens instantly and keeps working while a source site is down; titles are matched against TMDB in the background (needs a TMDB key) so “already in your library” is decided by ID rather than by name; hand the whole selection to the download queue or to Auto-Sync in one go
...
- **Home page** — an opt-in start page split into two tabs, because it was answering two different questions in one column. **Dashboard** is the glance: a button bar for queue, activity, library and — for admins — storage and system health, one panel at a time, plus any source that is not answering, the setup checklist and *your* rows (continue watching with resume-at-the-second, watchlist, newest library additions, what airs in the next two weeks, series with holes in them). **Discover** is the browsing half: **Could be for you** — the one row made of titles you do *not* own, tallied out of the TMDB recommendations already cached for the titles you do, so it costs no extra network calls and names which of your titles produced each suggestion — then *Because you watched X* and one *New* / *Popular* / *Movies* row mixing every enabled source, deduplicated across the whole page. Dropdowns above them say which sources are on, off or unreachable and filter at the same time; the search field sits in a sticky strip next to the tab switch and remembers your last searches (<kbd>/</kbd> jumps into it). Every row says where it comes from, and **Settings → Start Page** decides which rows you get, in which order and how many cards each holds — per account, on top of the default the admin sets
