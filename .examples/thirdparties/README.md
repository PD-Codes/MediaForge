...
- **"Do I already have this?" without re-implementing it.** `app.js` exposes
  `window.mfCardInLibrary(item)` and `window.mfCardOnAutoSync(item)` — the
  exact checks behind the two badges `renderBrowseCards()` puts on a card
  ("Vorhanden" and the Sync pill). Both take a card *item* (`{title, url,
  series_title?, tmdb?}`), not a DOM node, and both answer from data `app.js`
  has already loaded (`loadDownloadedFolders()` + `loadAutoSyncJobs()`), so
  call those first on a page that does not. Use them for filtering,
  sorting or a "hide what I have" toggle of your own — the home page's Status
  filter is built on exactly these two. Do not write your own title
  comparison: the library check is three passes deep (server-resolved alias
  index, strict folder prefix, punctuation-insensitive match) plus a separate
  Plex/Jellyfin path, and a copy of it drifts the first time any of those
  changes.
