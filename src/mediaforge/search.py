...
try:
    ...
    from mediaforge.config import GLOBAL_SESSION, logger
...
_homepage_cache = None        # cached HTML string
_homepage_cache_at = 0.0      # unix timestamp of last successful fetch
_homepage_cache_ttl = 3600.0  # 1 hour
_homepage_refreshing = False   # background refresh in progress
_homepage_lock = _threading.Lock()
_series_html_content = None
_series_lock = _threading.Lock()
...
def fetch_new_episodes():
    """Fetch the latest episodes from mediaforge.to/neue-episoden.
...
def _fetch_homepage():
    """Return cached homepage HTML with TTL + stale-while-revalidate."""
    global _homepage_cache, _homepage_cache_at, _homepage_refreshing

    # 1 + 2: single lock acquisition — compute `now` inside so _homepage_cache_at
    # is always compared to a timestamp taken under the same lock.
    with _homepage_lock:
        now = _time.time()
        if _homepage_cache is not None and (now - _homepage_cache_at) < _homepage_cache_ttl:
            return _homepage_cache

        # Stale but present — kick off a background refresh, return stale data now
        if _homepage_cache is not None:
            if not _homepage_refreshing:
                _homepage_refreshing = True

                def _bg_refresh():
                    global _homepage_cache, _homepage_cache_at, _homepage_refreshing
                    try:
                        resp = GLOBAL_SESSION.get(HOME_URL)
                        resp.raise_for_status()
                        with _homepage_lock:
                            _homepage_cache = resp.text
                            _homepage_cache_at = _time.time()
                    except Exception as exc:
                        logger.warning(f"Homepage background refresh failed: {exc}")
                    finally:
                        with _homepage_lock:
                            _homepage_refreshing = False

                _threading.Thread(target=_bg_refresh, daemon=True,
                                  name="homepage-refresh").start()
            return _homepage_cache

    # 3. Cold start — fetch synchronously (lock re-acquired after stale check)
    with _homepage_lock:
        if _homepage_cache is not None:
            return _homepage_cache
        try:
...
def _extract_cover_list(html, heading):
    ...
    for m in item_pattern.finditer(section_html):
        ...
        img_alt_text = html_module.unescape(img_alt_text)
            
        h3_match = re.search(r"<h3>(.*?)</h3>", inner, re.DOTALL)
        h3_text = re.sub(r"<[^>]+>", "", h3_match.group(1)).strip() if h3_match else ""
        h3_text = html_module.unescape(h3_text)
        
        # Prefer img_alt_text since it's the most reliable clean title
        title = img_alt_text if img_alt_text else h3_text
